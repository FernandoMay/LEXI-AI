#!/usr/bin/env python3
"""
LEXI AI — Pipeline Orchestrator
NLP Agent → Compliance Engine → Multi-Party Signatures → Audit Report
AST-bound cryptographic hashing that ties NLP analysis to on-chain evidence.
"""
import sys
import json
import hashlib
...
def run_pipeline(law_path: str = None, operational_context: dict = None) -> dict:
    ...
    # ── Agent 1: NLP Extractor (produces AST hash) ──────────────────────────
    nlp_agent = AgentTrace("LexiNLP", "0.1.0", "Hybrid: spaCy+HuggingFace+Regex")
    nlp_agent.start()
    nlp_agent.inputs["source"] = law_path or "mock"

    if law_path:
        nlp_raw = json.loads(extract_legal_axioms(law_path))
        rules = nlp_raw["axioms"]
        ast_hash = nlp_raw["ast_hash"]
        nlp_method = nlp_raw["method"]
    else:
        rules = [
        ...
        ast_digest = hashlib.sha256(json.dumps(rules, sort_keys=True).encode()).hexdigest()
        ast_hash = ast_digest
        nlp_method = "mock_regex"
    ...
    # ── AST-bound audit hash ─────────────────────────────────────────────────
    h = hashlib.sha256(ast_hash.encode())
    ...
    web3_agent = AgentTrace("LexiWeb3", "0.1.0", "Soroban Smart Contract (Stellar) — 2-of-3 Multi-Sig")
    web3_agent.start()
    web3_agent.inputs["contract_id"] = SOROBAN_CONTRACT["contract_id"]
    web3_agent.inputs["network"] = SOROBAN_CONTRACT["network"]

    onchain_tx = {
        "tx_hash": hashlib.sha256(audit_hash.encode() + b"SOROBAN_MULTISIG").hexdigest(),
        "block_confirmed": int(time.time()),
        "contract_id": SOROBAN_CONTRACT["contract_id"],
        "function": "register_audit",
        "required_signers": 2,
        "signers": [PARTIES["audited_entity"]["id"], PARTIES["independent_auditor"]["id"]],
        "args": {
            "doc_id": (law_path.split("/")[-1] if law_path else "mock_legal_v1").replace(".txt", ""),
            "audit_hash": audit_hash,
    ...
    web3_agent.complete({
        ...
        "multi_sig_scheme": "2-of-3 (Regulator, Entity, Auditor)",
    })
    ...
    report = {
        ...
        "agents": agents,
        "contract": {**SOROBAN_CONTRACT, "registration_tx": onchain_tx},
        "compliance": {
            "status": compliant,
            "audit_hash": audit_hash,
            "ast_hash": ast_hash,
...
def main():
    ...
    print(f"  AST Hash:       {report['compliance']['ast_hash']}")
